...
export function TeamMemberCard({ member }: TeamMemberCardProps) {
    return (
        <motion.div
            className="group relative rounded-3xl bg-[#18317B] overflow-hidden p-8 md:p-10 flex flex-col justify-end min-h-[220px]"
            variants={itemVariants}
            whileHover={{ y: -4 }}
            transition={{ duration: 0.3 }}
        >
            {/* Decorative background pattern */}
            <ChainPattern className="absolute -top-20 -right-40 w-[700px] h-[400px] opacity-[0.07] pointer-events-none" />
            ...
            {/* Elevate mark watermark */}
            <div className="absolute top-6 right-6 opacity-10">
                <svg viewBox="0 0 270 299" fill="none" className="w-10 h-10">
                    <path
            ...
            <div className="relative z-10">
                <h3 className="heading-text text-3xl md:text-4xl text-white mb-1">
                    {member.name}
                </h3>
                <p className="text-[#3DE1DA] font-light text-sm tracking-widest uppercase">
                    {member.role}
                </p>
            </div>
        </motion.div>
